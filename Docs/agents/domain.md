...
- **`Docs/adr/`**, if it exists — read ADRs that touch the area you're about to work in. The ADR
  freshness rule (body frozen at decision time, trust the header, current truth lives in `CONTEXT.md`)
  is already stated in `CLAUDE.md`'s Domain docs section — don't re-derive it here.
