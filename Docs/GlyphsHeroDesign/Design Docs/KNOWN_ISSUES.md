...
# Bugs
- [ ] **`Self` delivery no longer self-damages — likely revert (2026-06-23, ADR-0003).** The delivery-pattern split made damage uniform hex-occupancy filtered to hostiles. `DeliveryPattern.Self` covers the caster's own hex, but an enemy never stands there, so a `Self` payload now hits no one — silently killing the **deliberate self-hurt build-around** (a player intentionally taking damage to drive `OnSelfHit`/threshold builds). There should never be a target on the pawn's own hex, so hostile-occupancy can't express this. Fix = make `Self` a *self-targeted* delivery (hit the caster — an exception to hostile-occupancy, the friendly/self side of the axis the aura work will also need), not filter it by enemy team. Pre-split behaviour: `PayloadTargeting.Self` damaged `_pawn` directly.
