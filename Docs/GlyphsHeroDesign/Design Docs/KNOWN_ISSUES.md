...
- [x] adding and removing max resource mods changes the current value. **Resolved as intended (2026-08-09).** Diagnosed: `Resource`'s `MaxValue.OnTotalChanged` handler (`Resource.cs`) clamps `CurrentValue` to the new max whenever a stat mod changes it. Adding a max-boost never touches current (matches the "current is absolute, persists across fights" call from [[bug-loose-buffs-inert|2026-06-22]]); removing one clamps current down only if current currently exceeds the new (lower) max — the mathematically minimal possible loss (current can never exceed max). The two coherent alternatives were considered and rejected: full-heal-on-any-max-change (would make this moot but reverses the player-injuries-persist decision) vs. keep-injuries-and-accept-the-clamp (current behavior) — no in-between exists. Keeping the clamp. Not yet exercised by shipped content (no authored attachment uses LifeMax/ManaMax as a pawnStatMod yet).
