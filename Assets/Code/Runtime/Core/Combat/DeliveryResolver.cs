...
{
    /// <summary>
    /// Pure geometry for the Delivery Pattern axis (ADR-0002/0004, CONTEXT.md). Maps a firing
    ...
    /// affects. The set is team-agnostic; which pawns on it are hit is decided by the delivery's
    /// <see cref="Affinity"/> at the call site (see <see cref="DeliveryAffinity"/>). This stays a pure
    /// function with no pawns, registry, grid, or engine dependency, so it is unit-testable in isolation.
    ///
